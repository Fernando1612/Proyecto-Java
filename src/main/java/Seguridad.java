...
public class Seguridad {
    ...
    String res;
    public void validarUsuario(String usuario[], String usu, String con, String rango, int intentos){
        boolean encontrado = false;
        for (int i = 0; i < usuario.length; i++) {
            if(usuario[i].equalsIgnoreCase(usu) && usuario[i+1].equals(con) && usuario[i+2].equalsIgnoreCase(rango)){
                res = "Bienvenido " + usu;
                encontrado = true;
                JOptionPane.showMessageDialog(null, res, " Inicio de sesion", JOptionPane.INFORMATION_MESSAGE);
                intentos = 0;
                login.setIntentos(intentos);
                // una vez el usuiario entre, se abre el formulario padre
                new FrmInicioAdm().setVisible(true);
                break;
            }else if(usuario[i].equalsIgnoreCase(usu) && usuario[i+1].equals(con) && usuario[i+2].equalsIgnoreCase(rango)){
                res = "Bienvenido " + usu;
                encontrado = true;
                JOptionPane.showMessageDialog(null, res, " Inicio de sesion", JOptionPane.INFORMATION_MESSAGE);
                intentos = 0;
